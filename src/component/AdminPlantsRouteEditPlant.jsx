...
export default class AdminPlantsRouteEditPlant extends Component {
	...
	componentDidMount() {
		axios
			.get("http://localhost:4000/plants/" + this.props.match.params.id)					//before: ("http://localhost:4000/plants/")	
			.then((response) => {
				this.setState({
	...
	onSubmit(e) {
		...
		axios
			.post(
				"http://localhost:4000/plants/update/" + this.props.match.params.id,					//before: ("http://localhost:4000/plants/update/")	
				obj
			)
